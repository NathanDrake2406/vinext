...
type AppSsrHtmlTransformOptions = {
...
export type AppSsrRenderRuntime = {
  renderStaticMarkup(element: ReactNode): string;
  renderFinalHtmlStream(options: {
    element: ReactNode;
    renderOptions: SsrRenderOptions;
    waitForAllReady: boolean;
    pprFallbackShellSignal?: AbortSignal;
    shouldRecoverShellError: (error: unknown) => boolean;
    bootstrapModuleUrl?: string;
    scriptNonce?: string;
    transform: AppSsrHtmlTransformOptions;
  }): Promise<{
...
function renderInsertedHtml(
  ...
  for (const element of insertedElements) {
    try {
      insertedHTML += runtime.renderStaticMarkup(
...
export function createAppSsrEntry(runtime: AppSsrRenderRuntime) {
    ...
    return runWithNavigationContext(async () => {
      ...
      return runWithRootParamsScope(rootParams, async () => {
        try {
          ...
          // is populated by the time the target runtime resolves below.
          ...
          let reactLinkHeader = "";
          const maxHeadersLength =
            options?.reactMaxHeadersLength ?? DEFAULT_REACT_MAX_HEADERS_LENGTH;
          ...
          const getInsertedHTML = (): string => {
            const insertedHTML = renderInsertedHtml(runtime, renderServerInsertedHTML());
          ...
          const { htmlStream: transformedHtmlStream, shellErrorRecovered } =
            await runtime.renderFinalHtmlStream({
              element: ssrRoot,
              renderOptions,
              waitForAllReady: options?.waitForAllReady === true,
              pprFallbackShellSignal,
              shouldRecoverShellError(error) {
                return (
              ...
              transform: {
                rscEmbed,
                injectHTML: getInsertedHTML,
                injectAfterHeadOpenHTML: getBeforeInteractiveHeadHTML,
                inlineCssManifest,
                inlineCssPrependCss: inlineCssFontStyles,
                inlineCssPrependFallbackHTML: inlineCssFontStyleFallbackHTML,
                inlineCssScriptNonce: options?.scriptNonce,
              },
            });

          const finalStream = deferUntilStreamConsumed(transformedHtmlStream, cleanup);
  ...
  return { handleSsr, default: defaultExport };
}
