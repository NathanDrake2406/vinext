...
export function setupCloudflarePlatform(
  ...
  let finalWranglerFileName = "wrangler.jsonc";
  let needsKvNamespaceId = cloudflare.dataCache === "kv";
  if (!wranglerConfigPlan) {
    fs.writeFileSync(
    ...
    if (wranglerConfigPlan.changed) {
      fs.writeFileSync(wranglerConfigPlan.path, wranglerConfigPlan.code, "utf-8");
      generatedPlatformFiles.push(wranglerConfigPlan.fileName);
    }
    finalWranglerFileName = wranglerConfigPlan.fileName;
    needsKvNamespaceId = wranglerConfigPlan.needsKvNamespaceId;
  }
